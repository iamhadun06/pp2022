class Student:
    ...
    def displayStudentInformation(self):
        print("Student Id: %d, Student name: %s, Student dob: %s" % 
...
class Course:
    ...
    def displayCourseInformation(self):
        print("CourseId: {}, Course name: {}".format(self.courseId, 
self.courseName))
...
class Mark:
    ...
    def __eq__(self, other):
        return self.get_student_id() == other.get_student_id() and 
self.get_course_id() == other.get_course_id()

    def displayStudentMark(self, studentName):
        print("Student Id: {}, Student Name: {}, Student Mark: {} 
".format(self.studentId, studentName, self.grade))
...
def showMarkInformationForSpecificCourse(courseName, students, courses, 
grades):
    courseId = 0
    ...
    for j in range(len(grades)):
        if int(grades[j].get_course_id()) == int(courseId):
            for k in range(len(students)):
                if int(students[k].get_student_id()) == 
int(grades[j].get_student_id()):
...
if __name__ == "__main__":
    ...
    for k in range(len(grades)):
        print(grades[k].get_student_id(), grades[k].get_course_id(), 
grades[k].get_grade())
    print("Show mark for specific course \n")
    courseName = input("Enter course name ")
    showMarkInformationForSpecificCourse(courseName, students, courses, 
grades)
